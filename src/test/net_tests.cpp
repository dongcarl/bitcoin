...
BOOST_AUTO_TEST_CASE(net_rekey)
{
    ...
    deserializer.reset(new V2TransportDeserializer(mutable_k1, mutable_k2, mutable_session_id));
    ...
    for (unsigned int i = 0; i <= 100; i++) {
        ...
        if (rekey) {
            ...
            CHash256().Write(mutable_session_id.begin(), mutable_session_id.size()).Write(mutable_k1.data(), mutable_k1.size()).Finalize(mutable_k1.data());
            CHash256().Write(mutable_session_id.begin(), mutable_session_id.size()).Write(mutable_k2.data(), mutable_k2.size()).Finalize(mutable_k2.data());
