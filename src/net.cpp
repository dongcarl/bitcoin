    ...
    while (nBytes > 0) {
        ...
        if (m_deserializer->Complete()) {
            ...
            if (m_deserializer->ProtocolUpgradeDetected(data)) {
                ...
                m_deserializer = MakeUnique<V2TransportDeserializer>(V2TransportDeserializer(k_1_a, k_2_a));
...
int V1TransportDeserializer::readHeader(const char *pch, unsigned int nBytes, bool first_message)
{
    ...
    if (!m_v2_handshake && hdr.nMessageSize > MAX_SIZE || hdr.nMessageSize > MAX_PROTOCOL_MESSAGE_LENGTH) {
        return -1;
    }
