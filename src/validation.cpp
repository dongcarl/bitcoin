...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& chainparams, VersionBitsCache& versionbitscache);
...
static bool IsCurrentForFeeEstimation(CChainState& chainstate) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (chainstate.m_chain.Tip()->GetBlockTime() < count_seconds(GetTime<std::chrono::seconds>() - MAX_FEE_ESTIMATION_TIP_AGE))
        return false;
    if (chainstate.m_chain.Height() < chainstate.m_blockman.pindexBestHeader->nHeight - 1)
        return false;
    return true;
}
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    auto taproot_state = VersionBitsState(m_active_chainstate.m_chain.Tip(), params, Consensus::DEPLOYMENT_TAPROOT, m_active_chainstate.m_blockman.versionbitscache);
...
public:
    ...
    bool Condition(const CBlockIndex* pindex, const Consensus::Params& params) const override
    {
        return pindex->nHeight >= params.MinBIP9WarningHeight &&
               ...
               ((ComputeBlockVersion(pindex->pprev, params, m_versionbitscache) >> bit) & 1) == 0;
    }
};
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams, VersionBitsCache& versionbitscache) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    AssertLockHeld(cs_main);
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != m_blockman.m_block_index.end()) {
            if (it->second.GetAncestor(pindex->nHeight) == pindex &&
                ...
                fScriptChecks = (GetBlockProofEquivalentTime(*m_blockman.pindexBestHeader, *pindex, *m_blockman.pindexBestHeader, chainparams.GetConsensus()) <= 60 * 60 * 24 * 7 * 2);
            }
        }
    }
    ...
    unsigned int flags = GetBlockScriptFlags(pindex, chainparams.GetConsensus(), m_blockman.versionbitscache);
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    try {
        FlatFilePos blockPos = m_blockman.SaveBlockToDisk(block, pindex->nHeight, m_chain, chainparams, dbp);
        if (blockPos.IsNull()) {
...
bool CChainState::LoadBlockIndexDB(const CChainParams& chainparams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    m_blockman.m_block_tree->ReadFlag("prunedblockfiles", m_blockman.fHavePruned);
    if (m_blockman.fHavePruned)
...
bool CChainState::LoadGenesisBlock(const CChainParams& chainparams)
{
    ...
    try {
        ...
        FlatFilePos blockPos = m_blockman.SaveBlockToDisk(block, 0, m_chain, chainparams, nullptr);
        if (blockPos.IsNull())
