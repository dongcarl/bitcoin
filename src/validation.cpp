...
static void LimitMempoolSize(CTxMemPool& pool, CChainState& active_chainstate, size_t limit, std::chrono::seconds age)
    EXCLUSIVE_LOCKS_REQUIRED(pool.cs, ::cs_main)
{
    ...
    for (const COutPoint& removed : vNoSpendsRemaining)
        active_chainstate.CoinsTip().Uncache(removed);
}
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
                 unsigned int flags, PrecomputedTransactionData& txdata, CChainState& active_chainstate) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    AssertLockHeld(cs_main);
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (txFrom) {
            ...
            const Coin& coinFromDisk = active_chainstate.CoinsTip().AccessCoin(txin.prevout);
...
{
public:
    MemPoolAccept(CTxMemPool& mempool, CChainState& active_chainstate) : m_pool(mempool), m_active_chainstate(active_chainstate), m_view(&m_dummy), m_viewmempool(&active_chainstate.CoinsTip(), m_pool),
                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept, CAmount* fee_out)
{
    ...
    return AcceptToMemoryPoolWithTime(chainparams, pool, active_chainstate, state, tx, GetTime(), plTxnReplaced, bypass_limits, nAbsurdFee, test_accept, fee_out);
}
...
bool CChainState::ActivateBestChainStep(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    bool is_ibd = IsInitialBlockDownload();
    CBlockIndex* active_chain_tip = m_chain.Tip();
...
static bool NotifyHeaderTip(CChainState& active_chainstate) LOCKS_EXCLUDED(cs_main) {
    bool fNotify = false;
    ...
    CBlockIndex* pindexHeader = nullptr;
    {
        ...
        if (pindexHeader != pindexHeaderOld) {
            fNotify = true;
            fInitialBlockDownload = active_chainstate.IsInitialBlockDownload();
...
bool TestBlockValidity(BlockValidationState& state, const CChainParams& chainparams, CChainState& active_chainstate, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    AssertLockHeld(cs_main);
    assert(pindexPrev && pindexPrev == active_chainstate.m_chain.Tip());
    CCoinsViewCache viewNew(&active_chainstate.CoinsTip());
    ...
    if (!ContextualCheckBlockHeader(block, state, active_chainstate.m_blockman, chainparams, pindexPrev, GetAdjustedTime()))
        return error("%s: Consensus::ContextualCheckBlockHeader: %s", __func__, state.ToString());
    if (!CheckBlock(block, state, chainparams.GetConsensus(), fCheckPOW, fCheckMerkleRoot))
        return error("%s: Consensus::CheckBlock: %s", __func__, state.ToString());
    if (!ContextualCheckBlock(block, state, chainparams.GetConsensus(), pindexPrev))
        return error("%s: Consensus::ContextualCheckBlock: %s", __func__, state.ToString());
    if (!active_chainstate.ConnectBlock(block, state, &indexDummy, viewNew, chainparams, true))
        return false;
