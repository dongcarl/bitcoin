...
void PeerManager::ProcessOrphanTx(std::set<uint256>& orphan_work_set)
{
    ...
    while (!orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(::ChainstateActive(), m_mempool, state, porphanTx, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
                                         ...
                                         const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (!AlreadyHaveTx(GenTxid(/* is_wtxid=*/true, wtxid), m_mempool) &&
            AcceptToMemoryPool(::ChainstateActive(), m_mempool, state, ptx, &lRemovedTxn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            m_mempool.check(&::ChainstateActive().CoinsTip());
